...
function inspectComponentProps<T>(component: React.ComponentType<T>): void {
  // Handle both class components and function components
  const componentWithDefaults = component as any;
  const defaultProps = componentWithDefaults.defaultProps || {};
    ...
    visit(tree, (node: UnistNode) => {
      ...
      if (node.name === "ComponentSource") {
        ...
        try {
          ...
          if (srcPath) {
            ...
            if (!component) {
              console.warn(`Component "${name}" not found in default style`);
              return;
            }
      ...
      if (node.name === "ComponentPreview") {
        ...
        try {
          ...
          if (!component) {
            console.warn(
              `Component "${name}" not found in default style for ComponentPreview`
            );
            return;
          }
      ...
      if (node.name === "CodeFragment") {
        ...
        try {
          ...
          if (!component) {
            console.warn(
              `Component "${name}" not found in default style for CodeFragment`
            );
            return;
          }
          const src = component.files[0];
