...
async function generateAirdropTypes(
  ...
  const airdropTypesContent = `// Auto-generated from airdrop.config.mjs
// Do not edit this file manually

import type { AirdropProjectConfig } from './airdrop-config'
...
export type DevRevObject = ${config.devrevObjects.map(obj => toPascalCase(obj)).join(' | ')}

// Export all external sync unit types  
export type ExternalSyncUnit = ${config.externalSyncUnits.map(unit => toPascalCase(unit)).join(' | ')}
`

  const airdropTypesPath = path.join(typesDir, "airdrop.d.ts")
  await fs.writeFile(airdropTypesPath, airdropTypesContent, "utf8")
}
